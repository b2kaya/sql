...
--QUERY 2
SELECT 
	row_number () OVER (PARTITION BY customer_id ORDER BY market_date ASC) AS RowNumb,
	customer_id
...
SELECT
	...
	subquery.market_date
FROM
	(SELECT 
		row_number () OVER (PARTITION BY customer_id ORDER BY market_date DESC) AS RowNumb,
		customer_id,
		market_date
	FROM customer_purchases
	WHERE market_date < '2022-04-29') AS subquery
WHERE subquery.RowNumb = 1; 
...
--QUERY 10
INSERT INTO product_units(product_id, snapshot_timestamp)
VALUES ('Chocolate Cake', CURRENT_TIMESTAMP)
...
WHERE product_id = 'Chocolate Cake'
